...
class Watermark():
    def __init__(self, codebook_size=16384, bl_proportion=0.5):
        self.codebook_size = codebook_size
        self.bl_proportion = bl_proportion
        self.large_prime = 15485863
    
    def add_watermark(self, input_ids, distence):
        ...
        for b_idx in range(input_ids.shape[0]):         # 对一个batch的每个样本，循环执行以下代码
            for l_idx in range(1, input_ids.shape[1]):     # 遍历每一个索引进行加密
                # 以上一个索引元素来计算随机数种子
                ...
                bad_words_mask = torch.zeros(self.codebook_size)
                bad_words_mask[blacklist_ids] = 1       # 把黑名单对应位置索引标记为1
                bad_words_mask = bad_words_mask.bool()
                # 把对应黑名单的距离改为正无穷
                distence[b_idx, l_idx] = distence[b_idx, l_idx].masked_fill(bad_words_mask, float("inf"))
                idx_new = torch.argmin(distence[b_idx, l_idx])  # 计算新索引
...
class VectorQuantizer2(nn.Module):
    ...
    def __init__(self, n_e, e_dim, beta, remap=None, unknown_index="random",
        ...
        self.watermark = Watermark()    # 初始化水印添加
    ...
    def forward(self, z, temp=None, rescale_logits=False, return_logits=False):
        ...
        # 将索引写入文本，测试用
        # """
        # original_path = '/home/xiaoduo/felix_container/felix_project/clone_from_git/taming-transformers/original_index.txt'
        # watermark_path = '/home/xiaoduo/felix_container/felix_project/clone_from_git/taming-transformers/watermarked.txt'
        # min_encoding_indices_original = min_encoding_indices_original.view(b*h*w)
        # original_idx_np = min_encoding_indices_original.numpy()
        # watermarked_idx_np = min_encoding_indices.numpy()
        # np.savetxt(original_path, original_idx_np, fmt='%d', delimiter=' ')
